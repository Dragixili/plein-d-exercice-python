...
# ajouter le nouveau prix seulement s'il n'existe pas déjà
if x == 1:
    prix.append(price)
...
try:
    with open("prix_2.txt", "r") as f:
        for ligne in f:
            propre = re.sub(r"[^0-9.,]","",ligne.strip()).replace(",",".")
            if propre:
                prix_2.append(float(propre))  # convertir en nombre
except FileNotFoundError:
    pass

# ajouter le nouveau prix seulement s'il n'existe pas déjà
if x == 1:
    prix_2.append(price_2)


# réécrire le fichier
with open("prix_2.txt", "w") as f:
    for valeur in prix_2:
        f.write(str(valeur) + "\n")
        

plt.plot(date, prix, marker='o', label=title)
plt.plot(date, prix_2, marker='o', label=title_2)
